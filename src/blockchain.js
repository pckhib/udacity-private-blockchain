...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (currentTime - time < 5 * 60) {
        ...
        reject("It has been more than 5 minutes");
      }
    });
  }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      const filteredBlocks = self.chain.filter((value) => value.hash == hash);
      if (filteredBlocks.length > 0) {
        resolve(filteredBlocks[0]);
      } else {
        resolve(null);
      }
    });
  ...
  validateChain() {
    ...
    return new Promise(async (resolve, reject) => {
      for (let i = 0; i < self.chain.length - 1; i++) {
        ...
        const blockValidated = await block.validate();
        if (!blockValidated) {
